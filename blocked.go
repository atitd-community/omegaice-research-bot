...
func onBlocked(s *discordgo.Session, m *discordgo.MessageCreate) {
	if m.GuildID != "850171092941537313" {
		return
	}

	if !strings.HasPrefix(m.Message.Content, "!block") && !strings.HasPrefix(m.Message.Content, "!blocked") {
		return
	}
	...
	for _, line := range strings.Split(current, "\n") {
		if !skip {
			...
			if strings.TrimSpace(line) == "}}" {
				skip = false
				result = append(result, "{{LabResource|NEEDS UPDATE|0|1}}")
	...
	if !alreadyBlocked {
		...
		if _, err := s.ChannelMessageSend(m.ChannelID, fmt.Sprintf("Updated %s to mark %s as blocked.", page, parts[0][3])); err != nil {
			log.Println(err)
		}
	} else {
		if _, err := s.ChannelMessageSend(m.ChannelID, fmt.Sprintf("%s is already marked as blocked at %s.", parts[0][3], page)); err != nil {
			log.Println(err)
		}
	}
}
